    ...
    FUN = function(key) {
      ...
      switch(
        ...
        "phen_VegAndRepTherTimRes_bothX3" = {
          ...
          new_model <<- rapsimx::.replace_X(new_model, "[Phenology].VegetativeThermalTime.Response", 3, value)
          new_model <<- rapsimx::.replace_X(new_model, "[Phenology].ReproductiveThermalTime.Response", 3, value)
        },
        "phen_VegAndRepPhoMod_bothX1" = {
          ...
          # 14.43, 21.19
          new_model <<- rapsimx::.replace_X(new_model, "[Phenology].VegetativePhotoperiodModifier.XYPairs", 1, value)
          # 14.43, 21.19
          new_model <<- rapsimx::.replace_X(new_model, "[Phenology].ReproductivePhotoperiodModifier.XYPairs", 1, value)
        },
        ...
        "phen_VegetativeTarget" = {
          ...
          # 200.0
          new_model <<- rapsimx::.replace_FixedValue(new_model, "[Phenology].Vegetative.Target", value)
        },
        "phen_EarlyFloweringTarget" = {
          if (VERBOSE) print("Updating [Phenology].EarlyFlowering.Target")
          # 200.0
          new_model <<- rapsimx::.replace_FixedValue(new_model, "[Phenology].EarlyFlowering.Target", value)
        },
        "phen_EarlyPodDevTarget" = {
          if (VERBOSE) print("Updating [Phenology].EarlyPodDevelopment.Target")
          # 140.0
          new_model <<- rapsimx::.replace_FixedValue(new_model, "[Phenology].EarlyPodDevelopment.Target", value)
        },
        "phen_FractGrainFill" = {
          if (VERBOSE) print("Updating [Phenology].EarlyGrainFilling.Target.FractionofGrainfilling")
          # 0.05
          new_model <<- rapsimx::.replace_FixedValue(new_model, "[Phenology].EarlyGrainFilling.Target.FractionofGrainfilling", value)
        },
        "phen_EntGrainFill" = {
          if (VERBOSE) print("Updating [Phenology].LateGrainFilling.Target.EntireGrainfillPeriod")
          # 500.0
          new_model <<- rapsimx::.replace_FixedValue(new_model, "[Phenology].LateGrainFilling.Target.EntireGrainfillPeriod", value)
        },
        "phen_MidGrainFill" = {
          if (VERBOSE) print("Updating [Phenology].MidGrainFilling.Target.FractionofMidToLateGrainfilling")
          # 0.5
          new_model <<- rapsimx::.replace_FixedValue(new_model, "[Phenology].MidGrainFilling.Target.FractionofMidToLateGrainfilling", value)
        },
        "phen_Maturing" = {
          if (VERBOSE) print("Updating [Phenology].Maturing.Target")
          # 45.0
          new_model <<- rapsimx::.replace_FixedValue(new_model, "[Phenology].Maturing.Target", value)
        },
        "phen_Ripening" = {
          if (VERBOSE) print("Updating [Phenology].Ripening.Target")
          # 45.0
          new_model <<- rapsimx::.replace_FixedValue(new_model, "[Phenology].Ripening.Target", value)
        },
        "phen_shootlag" = {
          if (VERBOSE) print("Updating [Phenology].Emerging.Target.ShootLag")
          # 10
          new_model <<- rapsimx::.replace_FixedValue(new_model, "[Phenology].Emerging.Target.ShootLag", value)
        },
        "phen_shootrate" = {
          if (VERBOSE) print("Updating [Phenology].Emerging.Target.DepthxRate.ShootRate")
          # 1.0
          new_model <<- rapsimx::.replace_FixedValue(new_model, "[Phenology].Emerging.Target.DepthxRate.ShootRate", value)
        },
        "leaf_RUE" = {
          if (VERBOSE) print("Updating [Leaf].Photosynthesis.RUE")
          # 1.2
          new_model <<- rapsimx::.replace_FixedValue(new_model, "[Leaf].Photosynthesis.RUE", value)
        },
        "leaf_AreaLargLeaf" = {
          if (VERBOSE) print("Updating [Leaf].AreaLargestLeaf")
          # 0.006
          new_model <<- rapsimx::.replace_FixedValue(new_model, "[Leaf].AreaLargestLeaf", value)
        },
        "leaf_Phyllochron" = {
          if (VERBOSE) print("Updating [Leaf].Phyllochron")
          # 45.0
          new_model <<- rapsimx::.replace_FixedValue(new_model, "[Leaf].Phyllochron", value)
        },
        "leaf_ExtinctionCoef_Y1" = {
          if (VERBOSE) print("Updating [Leaf].ExtinctionCoefficient.XYPairs")
          # 0.6, 0.4
          new_model <<- rapsimx::.replace_Y(new_model, "[Leaf].ExtinctionCoefficient.XYPairs", 1, value)
        },
        "grain_HarvIndex" = {
          if (VERBOSE) print("Updating [Grain].PotentialHarvestIndex")
          # 0.5
          new_model <<- rapsimx::.replace_FixedValue(new_model, "[Grain].PotentialHarvestIndex", value)
        },
        "root_EarlyFrontVel" = {
          if (VERBOSE) print("Updating [Root].RootFrontVelocity.PotentialRootFrontVelocity.early.Function")
          # 30.0
          new_model <<- rapsimx::.replace_FixedValue(new_model, "[Root].RootFrontVelocity.PotentialRootFrontVelocity.early.Function", value)
        },
        "root_LateFrontVel" = {
          if (VERBOSE) print("Updating [Root].RootFrontVelocity.PotentialRootFrontVelocity.late.Function")
          # 5.0
          new_model <<- rapsimx::.replace_FixedValue(new_model, "[Root].RootFrontVelocity.PotentialRootFrontVelocity.late.Function", value)
        },
        "nodule_VegGrowthRate" = {
          ...
          # 0.006
          new_model <<- rapsimx::.replace_FixedValue(new_model, "[Nodule].FixationRate.DailyPotentialFixationRate.PotentialFixationRate.SpecificFixationRate.VegetativeGrowth.Rate", value)
        },
        "nodule_RepGrowthRate" = {
          ...
          # 0.002
          new_model <<- rapsimx::.replace_FixedValue(new_model, "[Nodule].FixationRate.DailyPotentialFixationRate.PotentialFixationRate.SpecificFixationRate.ReproductiveGrowth.Rate", value)
        },
        "nodule_MaxFixRate" = {
          if (VERBOSE) print("Updating [Nodule].FixationRate.DailyPotentialFixationRate.MaximumFixationRate")
          # 0.6
          new_model <<- rapsimx::.replace_FixedValue(new_model, "[Nodule].FixationRate.DailyPotentialFixationRate.MaximumFixationRate", value)
        },
        {
